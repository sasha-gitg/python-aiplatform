...
class CustomTrainingJob(base.AiPlatformResourceNoun):
    ...
    def run(
        ...
        staging_bucket = self._staging_bucket or initializer.global_config.staging_bucket
        ...
        if model_display_name and not all([self._model_serving_container_image_uri,
    ...
    def _block_until_complete(self):
        ...
        while self.state not in _PIPELINE_COMPLETE_STATES:
            ...
            time.sleep(wait)
            _LOGGER.info("Training %s current state:\n%s" %
    ...
    def _model_upload_fail_string(self) -> str:
        ...
        return (f"Training Pipeline {self.resource_name} is not configured to upload a "
